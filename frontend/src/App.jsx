...
/**
 * @component ApprovalItemCard
 * @description Renders a single expense item and handles the approval/rejection action logic.
 */
const ApprovalItemCard = ({ expense, companyCurrency, onAction, setNotification }) => {
    const [comment, setComment] = useState('');
    const [isProcessing, setIsProcessing] = useState(false);

    const handleAction = async (finalAction) => {
        setIsProcessing(true);
        try {
            // This calls the handler function passed down from ExpensesForApproval
            await onAction(expense.id, finalAction, comment);
            setNotification(`Expense for ${expense.employee} ${finalAction}ed!`);
        } catch (error) {
            setNotification(`Action failed for expense ${expense.id}.`);
        } finally {
    ...
    return (
        <div className="border border-indigo-200 p-4 rounded-xl shadow-md bg-white">
            ...
            <p className="text-sm text-gray-600 mb-2">
                Submitted by: **{expense.employee}** | Category: {expense.category}
            </p>
...
/**
 * @component ExpensesForApproval
 * @description Container component for the approval view. Handles data fetching and state.
 */
const ExpensesForApproval = () => {
    ...
    const fetchPending = async () => {
        ...
        const mockPending = [
            { id: 101, employee: 'Jane Doe', category: 'Meal', description: 'Client Dinner', amount: 150, currency: 'USD', convertedAmount: 150.00, flowApplied: 'Default', status: 'Pending' },
            { id: 102, employee: 'John Smith', category: 'Travel', description: 'Train ticket', amount: 450, currency: 'EUR', convertedAmount: 485.50, flowApplied: 'High Value', status: 'Pending' },
            { id: 103, employee: 'Alice Brown', category: 'Software', description: 'Annual SaaS subscription', amount: 1200, currency: 'USD', convertedAmount: 1200.00, flowApplied: 'CFO Flow', status: 'Pending' },
        ];
    ...
    // Handler passed down to child component to execute API action
    const handleApprovalAction = async (expenseId, finalAction, comment) => {
        // This calls the backend endpoint: PUT /api/expenses/:id/action
        const response = await api(`/expenses/${expenseId}/action`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: finalAction, comment: comment })
        });
        
        if (response.success) {
            // Remove the expense from the local pending list instantly for good UX
            setPendingExpenses(prev => prev.filter(exp => exp.id !== expenseId));
            // Trigger notification state update
            setTimeout(() => setNotification(''), 3000); 
        } else {
            throw new Error('Approval action failed on server.');
        }
    };
